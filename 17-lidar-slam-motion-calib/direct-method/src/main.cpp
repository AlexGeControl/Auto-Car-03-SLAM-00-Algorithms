...
#include "/home/ad/Workspace/05-lidar-slam/HW2/csm-master/deploy/include/csm/csm_all.h"
...
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    double c, s;
    Eigen::Matrix3d T_last;
    c = cos(last_pos(2));
    s = sin(last_pos(2));
    T_last << c, -s, last_pos(0),
	      s,  c, last_pos(1),
              0,  0,           1;
    ...
    Eigen::Matrix3d T = T_last.inverse() * T_now;
    d_pos(0) = T(0, 2);
    d_pos(1) = T(1, 2);
